...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def go(cur: Int, nxt: Int, n: Int): Int = {
      if(n == 0) cur else go(nxt, cur + nxt, n-1)
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = as.zip(as.slice(1, as.length)).forall(cur => gt(cur._1, cur._2))
